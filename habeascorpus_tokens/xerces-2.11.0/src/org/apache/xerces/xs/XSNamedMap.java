...
;	TokenNameSEMICOLON	
/** * Objects implementing the <code>XSNamedMap</code> interface are used to * represent immutable collections of XML Schema components that can be * accessed by name. Note that <code>XSNamedMap</code> does not inherit from * <code>XSObjectList</code>. The <code>XSObject</code>s in * <code>XSNamedMap</code>s are not maintained in any particular order. */	TokenNameCOMMENT_JAVADOC	 Objects implementing the <code>XSNamedMap</code> interface are used to represent immutable collections of XML Schema components that can be accessed by name. Note that <code>XSNamedMap</code> does not inherit from <code>XSObjectList</code>. The <code>XSObject</code>s in <code>XSNamedMap</code>s are not maintained in any particular order. 
