...
;	TokenNameSEMICOLON	
/** * Tests for {@link KStemmer} */	TokenNameCOMMENT_JAVADOC	 Tests for {@link KStemmer} 
public	TokenNamepublic	
class	TokenNameclass	
TestKStemmer	TokenNameIdentifier	 Test K Stemmer
...
,	TokenNameCOMMA	
new	TokenNamenew	
KStemFilter	TokenNameIdentifier	 K Stem Filter
(	TokenNameLPAREN	
tokenizer	TokenNameIdentifier	 tokenizer
)	TokenNameRPAREN	
)	TokenNameRPAREN	
...
/** * test the kstemmer optimizations against a bunch of words * that were stemmed with the original java kstemmer (generated from * testCreateMap, commented out below). */	TokenNameCOMMENT_JAVADOC	 test the kstemmer optimizations against a bunch of words that were stemmed with the original java kstemmer (generated from testCreateMap, commented out below). 
public	TokenNamepublic	
void	TokenNamevoid	
...
(	TokenNameLPAREN	
"kstemTestData.zip"	TokenNameStringLiteral	kstemTestData.zip
)	TokenNameRPAREN	
,	TokenNameCOMMA	
"kstem_examples.txt"	TokenNameStringLiteral	kstem_examples.txt
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
}	TokenNameRBRACE	
public	TokenNamepublic	
...
/****** requires original java kstem source code to create map public void testCreateMap() throws Exception { String input = getBigDoc(); Reader r = new StringReader(input); TokenFilter tf = new LowerCaseFilter(new LetterTokenizer(r)); // tf = new KStemFilter(tf); KStemmer kstem = new KStemmer(); Map<String,String> map = new TreeMap<String,String>(); for(;;) { Token t = tf.next(); if (t==null) break; String s = t.termText(); if (map.containsKey(s)) continue; map.put(s, kstem.stem(s)); } Writer out = new BufferedWriter(new FileWriter("kstem_examples.txt")); for (String key : map.keySet()) { out.write(key); out.write('\t'); out.write(map.get(key)); out.write('\n'); } out.close(); } ******/	TokenNameCOMMENT_JAVADOC	**** requires original java kstem source code to create map public void testCreateMap() throws Exception { String input = getBigDoc(); Reader r = new StringReader(input); TokenFilter tf = new LowerCaseFilter(new LetterTokenizer(r)); // tf = new KStemFilter(tf); KStemmer kstem = new KStemmer(); Map<String,String> map = new TreeMap<String,String>(); for(;;) { Token t = tf.next(); if (t==null) break; String s = t.termText(); if (map.containsKey(s)) continue; map.put(s, kstem.stem(s)); } Writer out = new BufferedWriter(new FileWriter("kstem_examples.txt")); for (String key : map.keySet()) { out.write(key); out.write('\t'); out.write(map.get(key)); out.write('\n'); } out.close(); } *****
}	TokenNameRBRACE	
