/* Copyright 2003, Carnegie Mellon, All Rights Reserved */	TokenNameCOMMENT_BLOCK	 Copyright 2003, Carnegie Mellon, All Rights Reserved 
package	TokenNamepackage	
edu	TokenNameIdentifier	 edu
.	TokenNameDOT	
cmu	TokenNameIdentifier	 cmu
.	TokenNameDOT	
minorthird	TokenNameIdentifier	 minorthird
.	TokenNameDOT	
classify	TokenNameIdentifier	 classify
.	TokenNameDOT	
multi	TokenNameIdentifier	 multi
;	TokenNameSEMICOLON	
import	TokenNameimport	
java	TokenNameIdentifier	 java
.	TokenNameDOT	
awt	TokenNameIdentifier	 awt
.	TokenNameDOT	
Component	TokenNameIdentifier	 Component
;	TokenNameSEMICOLON	
import	TokenNameimport	
java	TokenNameIdentifier	 java
.	TokenNameDOT	
util	TokenNameIdentifier	 util
.	TokenNameDOT	
Iterator	TokenNameIdentifier	 Iterator
;	TokenNameSEMICOLON	
import	TokenNameimport	
javax	TokenNameIdentifier	 javax
.	TokenNameDOT	
swing	TokenNameIdentifier	 swing
.	TokenNameDOT	
ButtonGroup	TokenNameIdentifier	 Button Group
;	TokenNameSEMICOLON	
import	TokenNameimport	
javax	TokenNameIdentifier	 javax
.	TokenNameDOT	
swing	TokenNameIdentifier	 swing
.	TokenNameDOT	
JCheckBox	TokenNameIdentifier	 J Check Box
;	TokenNameSEMICOLON	
import	TokenNameimport	
javax	TokenNameIdentifier	 javax
.	TokenNameDOT	
swing	TokenNameIdentifier	 swing
.	TokenNameDOT	
JComponent	TokenNameIdentifier	 J Component
;	TokenNameSEMICOLON	
import	TokenNameimport	
javax	TokenNameIdentifier	 javax
.	TokenNameDOT	
swing	TokenNameIdentifier	 swing
.	TokenNameDOT	
JRadioButton	TokenNameIdentifier	 J Radio Button
;	TokenNameSEMICOLON	
import	TokenNameimport	
javax	TokenNameIdentifier	 javax
.	TokenNameDOT	
swing	TokenNameIdentifier	 swing
.	TokenNameDOT	
JScrollPane	TokenNameIdentifier	 J Scroll Pane
;	TokenNameSEMICOLON	
import	TokenNameimport	
javax	TokenNameIdentifier	 javax
.	TokenNameDOT	
swing	TokenNameIdentifier	 swing
.	TokenNameDOT	
JSplitPane	TokenNameIdentifier	 J Split Pane
;	TokenNameSEMICOLON	
import	TokenNameimport	
javax	TokenNameIdentifier	 javax
.	TokenNameDOT	
swing	TokenNameIdentifier	 swing
.	TokenNameDOT	
JTabbedPane	TokenNameIdentifier	 J Tabbed Pane
;	TokenNameSEMICOLON	
import	TokenNameimport	
javax	TokenNameIdentifier	 javax
.	TokenNameDOT	
swing	TokenNameIdentifier	 swing
.	TokenNameDOT	
JTable	TokenNameIdentifier	 J Table
;	TokenNameSEMICOLON	
import	TokenNameimport	
javax	TokenNameIdentifier	 javax
.	TokenNameDOT	
swing	TokenNameIdentifier	 swing
.	TokenNameDOT	
ScrollPaneConstants	TokenNameIdentifier	 Scroll Pane Constants
;	TokenNameSEMICOLON	
import	TokenNameimport	
javax	TokenNameIdentifier	 javax
.	TokenNameDOT	
swing	TokenNameIdentifier	 swing
.	TokenNameDOT	
table	TokenNameIdentifier	 table
.	TokenNameDOT	
AbstractTableModel	TokenNameIdentifier	 Abstract Table Model
;	TokenNameSEMICOLON	
import	TokenNameimport	
javax	TokenNameIdentifier	 javax
.	TokenNameDOT	
swing	TokenNameIdentifier	 swing
.	TokenNameDOT	
table	TokenNameIdentifier	 table
.	TokenNameDOT	
TableCellRenderer	TokenNameIdentifier	 Table Cell Renderer
;	TokenNameSEMICOLON	
import	TokenNameimport	
edu	TokenNameIdentifier	 edu
.	TokenNameDOT	
cmu	TokenNameIdentifier	 cmu
.	TokenNameDOT	
minorthird	TokenNameIdentifier	 minorthird
.	TokenNameDOT	
classify	TokenNameIdentifier	 classify
.	TokenNameDOT	
Explanation	TokenNameIdentifier	 Explanation
;	TokenNameSEMICOLON	
import	TokenNameimport	
edu	TokenNameIdentifier	 edu
.	TokenNameDOT	
cmu	TokenNameIdentifier	 cmu
.	TokenNameDOT	
minorthird	TokenNameIdentifier	 minorthird
.	TokenNameDOT	
classify	TokenNameIdentifier	 classify
.	TokenNameDOT	
Feature	TokenNameIdentifier	 Feature
;	TokenNameSEMICOLON	
import	TokenNameimport	
edu	TokenNameIdentifier	 edu
.	TokenNameDOT	
cmu	TokenNameIdentifier	 cmu
.	TokenNameDOT	
minorthird	TokenNameIdentifier	 minorthird
.	TokenNameDOT	
util	TokenNameIdentifier	 util
.	TokenNameDOT	
ProgressCounter	TokenNameIdentifier	 Progress Counter
;	TokenNameSEMICOLON	
import	TokenNameimport	
edu	TokenNameIdentifier	 edu
.	TokenNameDOT	
cmu	TokenNameIdentifier	 cmu
.	TokenNameDOT	
minorthird	TokenNameIdentifier	 minorthird
.	TokenNameDOT	
util	TokenNameIdentifier	 util
.	TokenNameDOT	
gui	TokenNameIdentifier	 gui
.	TokenNameDOT	
ComponentViewer	TokenNameIdentifier	 Component Viewer
;	TokenNameSEMICOLON	
import	TokenNameimport	
edu	TokenNameIdentifier	 edu
.	TokenNameDOT	
cmu	TokenNameIdentifier	 cmu
.	TokenNameDOT	
minorthird	TokenNameIdentifier	 minorthird
.	TokenNameDOT	
util	TokenNameIdentifier	 util
.	TokenNameDOT	
gui	TokenNameIdentifier	 gui
.	TokenNameDOT	
Controllable	TokenNameIdentifier	 Controllable
;	TokenNameSEMICOLON	
import	TokenNameimport	
edu	TokenNameIdentifier	 edu
.	TokenNameDOT	
cmu	TokenNameIdentifier	 cmu
.	TokenNameDOT	
minorthird	TokenNameIdentifier	 minorthird
.	TokenNameDOT	
util	TokenNameIdentifier	 util
.	TokenNameDOT	
gui	TokenNameIdentifier	 gui
.	TokenNameDOT	
ControlledViewer	TokenNameIdentifier	 Controlled Viewer
;	TokenNameSEMICOLON	
import	TokenNameimport	
edu	TokenNameIdentifier	 edu
.	TokenNameDOT	
cmu	TokenNameIdentifier	 cmu
.	TokenNameDOT	
minorthird	TokenNameIdentifier	 minorthird
.	TokenNameDOT	
util	TokenNameIdentifier	 util
.	TokenNameDOT	
gui	TokenNameIdentifier	 gui
.	TokenNameDOT	
MessageViewer	TokenNameIdentifier	 Message Viewer
;	TokenNameSEMICOLON	
import	TokenNameimport	
edu	TokenNameIdentifier	 edu
.	TokenNameDOT	
cmu	TokenNameIdentifier	 cmu
.	TokenNameDOT	
minorthird	TokenNameIdentifier	 minorthird
.	TokenNameDOT	
util	TokenNameIdentifier	 util
.	TokenNameDOT	
gui	TokenNameIdentifier	 gui
.	TokenNameDOT	
VanillaViewer	TokenNameIdentifier	 Vanilla Viewer
;	TokenNameSEMICOLON	
import	TokenNameimport	
edu	TokenNameIdentifier	 edu
.	TokenNameDOT	
cmu	TokenNameIdentifier	 cmu
.	TokenNameDOT	
minorthird	TokenNameIdentifier	 minorthird
.	TokenNameDOT	
util	TokenNameIdentifier	 util
.	TokenNameDOT	
gui	TokenNameIdentifier	 gui
.	TokenNameDOT	
Viewer	TokenNameIdentifier	 Viewer
;	TokenNameSEMICOLON	
import	TokenNameimport	
edu	TokenNameIdentifier	 edu
.	TokenNameDOT	
cmu	TokenNameIdentifier	 cmu
.	TokenNameDOT	
minorthird	TokenNameIdentifier	 minorthird
.	TokenNameDOT	
util	TokenNameIdentifier	 util
.	TokenNameDOT	
gui	TokenNameIdentifier	 gui
.	TokenNameDOT	
ViewerControls	TokenNameIdentifier	 Viewer Controls
;	TokenNameSEMICOLON	
import	TokenNameimport	
edu	TokenNameIdentifier	 edu
.	TokenNameDOT	
cmu	TokenNameIdentifier	 cmu
.	TokenNameDOT	
minorthird	TokenNameIdentifier	 minorthird
.	TokenNameDOT	
util	TokenNameIdentifier	 util
.	TokenNameDOT	
gui	TokenNameIdentifier	 gui
.	TokenNameDOT	
Visible	TokenNameIdentifier	 Visible
;	TokenNameSEMICOLON	
/** Pairs a dataset and a classifier, for easy inspection * of the actions of a classifier. * * @author Cameron Williams */	TokenNameCOMMENT_JAVADOC	 Pairs a dataset and a classifier, for easy inspection of the actions of a classifier. * @author Cameron Williams 
public	TokenNamepublic	
class	TokenNameclass	
MultiClassifiedDataset	TokenNameIdentifier	 Multi Classified Dataset
implements	TokenNameimplements	
Visible	TokenNameIdentifier	 Visible
{	TokenNameLBRACE	
private	TokenNameprivate	
MultiClassifier	TokenNameIdentifier	 Multi Classifier
classifier	TokenNameIdentifier	 classifier
;	TokenNameSEMICOLON	
private	TokenNameprivate	
MultiDataset	TokenNameIdentifier	 Multi Dataset
dataset	TokenNameIdentifier	 dataset
;	TokenNameSEMICOLON	
private	TokenNameprivate	
MultiDatasetIndex	TokenNameIdentifier	 Multi Dataset Index
index	TokenNameIdentifier	 index
;	TokenNameSEMICOLON	
public	TokenNamepublic	
MultiClassifiedDataset	TokenNameIdentifier	 Multi Classified Dataset
(	TokenNameLPAREN	
MultiClassifier	TokenNameIdentifier	 Multi Classifier
classifier	TokenNameIdentifier	 classifier
,	TokenNameCOMMA	
MultiDataset	TokenNameIdentifier	 Multi Dataset
dataset	TokenNameIdentifier	 dataset
,	TokenNameCOMMA	
MultiDatasetIndex	TokenNameIdentifier	 Multi Dataset Index
index	TokenNameIdentifier	 index
)	TokenNameRPAREN	
{	TokenNameLBRACE	
this	TokenNamethis	
.	TokenNameDOT	
classifier	TokenNameIdentifier	 classifier
=	TokenNameEQUAL	
classifier	TokenNameIdentifier	 classifier
;	TokenNameSEMICOLON	
this	TokenNamethis	
.	TokenNameDOT	
dataset	TokenNameIdentifier	 dataset
=	TokenNameEQUAL	
dataset	TokenNameIdentifier	 dataset
;	TokenNameSEMICOLON	
this	TokenNamethis	
.	TokenNameDOT	
index	TokenNameIdentifier	 index
=	TokenNameEQUAL	
index	TokenNameIdentifier	 index
;	TokenNameSEMICOLON	
}	TokenNameRBRACE	
public	TokenNamepublic	
MultiClassifiedDataset	TokenNameIdentifier	 Multi Classified Dataset
(	TokenNameLPAREN	
MultiClassifier	TokenNameIdentifier	 Multi Classifier
classifier	TokenNameIdentifier	 classifier
,	TokenNameCOMMA	
MultiDataset	TokenNameIdentifier	 Multi Dataset
dataset	TokenNameIdentifier	 dataset
)	TokenNameRPAREN	
{	TokenNameLBRACE	
this	TokenNamethis	
(	TokenNameLPAREN	
classifier	TokenNameIdentifier	 classifier
,	TokenNameCOMMA	
dataset	TokenNameIdentifier	 dataset
,	TokenNameCOMMA	
new	TokenNamenew	
MultiDatasetIndex	TokenNameIdentifier	 Multi Dataset Index
(	TokenNameLPAREN	
dataset	TokenNameIdentifier	 dataset
)	TokenNameRPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
}	TokenNameRBRACE	
public	TokenNamepublic	
MultiClassifier	TokenNameIdentifier	 Multi Classifier
getClassifier	TokenNameIdentifier	 get Classifier
(	TokenNameLPAREN	
)	TokenNameRPAREN	
{	TokenNameLBRACE	
return	TokenNamereturn	
classifier	TokenNameIdentifier	 classifier
;	TokenNameSEMICOLON	
}	TokenNameRBRACE	
public	TokenNamepublic	
MultiDataset	TokenNameIdentifier	 Multi Dataset
getDataset	TokenNameIdentifier	 get Dataset
(	TokenNameLPAREN	
)	TokenNameRPAREN	
{	TokenNameLBRACE	
return	TokenNamereturn	
dataset	TokenNameIdentifier	 dataset
;	TokenNameSEMICOLON	
}	TokenNameRBRACE	
@	TokenNameAT	
Override	TokenNameIdentifier	 Override
public	TokenNamepublic	
Viewer	TokenNameIdentifier	 Viewer
toGUI	TokenNameIdentifier	 to GUI
(	TokenNameLPAREN	
)	TokenNameRPAREN	
{	TokenNameLBRACE	
Viewer	TokenNameIdentifier	 Viewer
v	TokenNameIdentifier	 v
=	TokenNameEQUAL	
new	TokenNamenew	
MessageViewer	TokenNameIdentifier	 Message Viewer
(	TokenNameLPAREN	
new	TokenNamenew	
MyViewer	TokenNameIdentifier	 My Viewer
(	TokenNameLPAREN	
)	TokenNameRPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
v	TokenNameIdentifier	 v
.	TokenNameDOT	
setContent	TokenNameIdentifier	 set Content
(	TokenNameLPAREN	
this	TokenNamethis	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
return	TokenNamereturn	
v	TokenNameIdentifier	 v
;	TokenNameSEMICOLON	
}	TokenNameRBRACE	
/** A toolbar to govern how data is filtered. */	TokenNameCOMMENT_JAVADOC	 A toolbar to govern how data is filtered. 
private	TokenNameprivate	
static	TokenNamestatic	
class	TokenNameclass	
DataControls	TokenNameIdentifier	 Data Controls
extends	TokenNameextends	
ViewerControls	TokenNameIdentifier	 Viewer Controls
{	TokenNameLBRACE	
static	TokenNamestatic	
final	TokenNamefinal	
long	TokenNamelong	
serialVersionUID	TokenNameIdentifier	 serial Version UID
=	TokenNameEQUAL	
20080130L	TokenNameLongLiteral	
;	TokenNameSEMICOLON	
public	TokenNamepublic	
JCheckBox	TokenNameIdentifier	 J Check Box
filterOnFeatureBox	TokenNameIdentifier	 filter On Feature Box
;	TokenNameSEMICOLON	
public	TokenNamepublic	
Feature	TokenNameIdentifier	 Feature
targetFeature	TokenNameIdentifier	 target Feature
;	TokenNameSEMICOLON	
public	TokenNamepublic	
JRadioButton	TokenNameIdentifier	 J Radio Button
correctButton	TokenNameIdentifier	 correct Button
;	TokenNameSEMICOLON	
//public JRadioButton incorrectButton; 	TokenNameCOMMENT_LINE	public JRadioButton incorrectButton; 
public	TokenNamepublic	
JRadioButton	TokenNameIdentifier	 J Radio Button
allButton	TokenNameIdentifier	 all Button
;	TokenNameSEMICOLON	
@	TokenNameAT	
Override	TokenNameIdentifier	 Override
public	TokenNamepublic	
void	TokenNamevoid	
initialize	TokenNameIdentifier	 initialize
(	TokenNameLPAREN	
)	TokenNameRPAREN	
{	TokenNameLBRACE	
// indicates if we should filter on some feature 	TokenNameCOMMENT_LINE	indicates if we should filter on some feature 
filterOnFeatureBox	TokenNameIdentifier	 filter On Feature Box
=	TokenNameEQUAL	
new	TokenNamenew	
JCheckBox	TokenNameIdentifier	 J Check Box
(	TokenNameLPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
filterOnFeatureBox	TokenNameIdentifier	 filter On Feature Box
.	TokenNameDOT	
setText	TokenNameIdentifier	 set Text
(	TokenNameLPAREN	
"[none]"	TokenNameStringLiteral	[none]
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
targetFeature	TokenNameIdentifier	 target Feature
=	TokenNameEQUAL	
null	TokenNamenull	
;	TokenNameSEMICOLON	
add	TokenNameIdentifier	 add
(	TokenNameLPAREN	
filterOnFeatureBox	TokenNameIdentifier	 filter On Feature Box
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
ButtonGroup	TokenNameIdentifier	 Button Group
group	TokenNameIdentifier	 group
=	TokenNameEQUAL	
new	TokenNamenew	
ButtonGroup	TokenNameIdentifier	 Button Group
(	TokenNameLPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
correctButton	TokenNameIdentifier	 correct Button
=	TokenNameEQUAL	
addButton	TokenNameIdentifier	 add Button
(	TokenNameLPAREN	
"correct"	TokenNameStringLiteral	correct
,	TokenNameCOMMA	
false	TokenNamefalse	
,	TokenNameCOMMA	
group	TokenNameIdentifier	 group
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
//incorrectButton=addButton("incorrect",false,group); 	TokenNameCOMMENT_LINE	incorrectButton=addButton("incorrect",false,group); 
allButton	TokenNameIdentifier	 all Button
=	TokenNameEQUAL	
addButton	TokenNameIdentifier	 add Button
(	TokenNameLPAREN	
"all"	TokenNameStringLiteral	all
,	TokenNameCOMMA	
true	TokenNametrue	
,	TokenNameCOMMA	
group	TokenNameIdentifier	 group
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
addApplyButton	TokenNameIdentifier	 add Apply Button
(	TokenNameLPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
}	TokenNameRBRACE	
private	TokenNameprivate	
JRadioButton	TokenNameIdentifier	 J Radio Button
addButton	TokenNameIdentifier	 add Button
(	TokenNameLPAREN	
String	TokenNameIdentifier	 String
s	TokenNameIdentifier	 s
,	TokenNameCOMMA	
boolean	TokenNameboolean	
selected	TokenNameIdentifier	 selected
,	TokenNameCOMMA	
ButtonGroup	TokenNameIdentifier	 Button Group
group	TokenNameIdentifier	 group
)	TokenNameRPAREN	
{	TokenNameLBRACE	
JRadioButton	TokenNameIdentifier	 J Radio Button
button	TokenNameIdentifier	 button
=	TokenNameEQUAL	
new	TokenNamenew	
JRadioButton	TokenNameIdentifier	 J Radio Button
(	TokenNameLPAREN	
s	TokenNameIdentifier	 s
,	TokenNameCOMMA	
selected	TokenNameIdentifier	 selected
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
group	TokenNameIdentifier	 group
.	TokenNameDOT	
add	TokenNameIdentifier	 add
(	TokenNameLPAREN	
button	TokenNameIdentifier	 button
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
add	TokenNameIdentifier	 add
(	TokenNameLPAREN	
button	TokenNameIdentifier	 button
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
return	TokenNamereturn	
button	TokenNameIdentifier	 button
;	TokenNameSEMICOLON	
}	TokenNameRBRACE	
}	TokenNameRBRACE	
/** A toolbar-controlled viewer for data/classifications in a classified dataset */	TokenNameCOMMENT_JAVADOC	 A toolbar-controlled viewer for data/classifications in a classified dataset 
static	TokenNamestatic	
private	TokenNameprivate	
class	TokenNameclass	
ControlledDataViewer	TokenNameIdentifier	 Controlled Data Viewer
extends	TokenNameextends	
ComponentViewer	TokenNameIdentifier	 Component Viewer
implements	TokenNameimplements	
Controllable	TokenNameIdentifier	 Controllable
{	TokenNameLBRACE	
static	TokenNamestatic	
final	TokenNamefinal	
long	TokenNamelong	
serialVersionUID	TokenNameIdentifier	 serial Version UID
=	TokenNameEQUAL	
20080130L	TokenNameLongLiteral	
;	TokenNameSEMICOLON	
// cached last display 	TokenNameCOMMENT_LINE	cached last display 
private	TokenNameprivate	
MultiClassifiedDataset	TokenNameIdentifier	 Multi Classified Dataset
cd	TokenNameIdentifier	 cd
;	TokenNameSEMICOLON	
// If true, only show example for which the classification is 	TokenNameCOMMENT_LINE	If true, only show example for which the classification is 
// correct (or incorrect) depending on targetCorrectness 	TokenNameCOMMENT_LINE	correct (or incorrect) depending on targetCorrectness 
private	TokenNameprivate	
boolean	TokenNameboolean	
filterOnCorrectness	TokenNameIdentifier	 filter On Correctness
=	TokenNameEQUAL	
false	TokenNamefalse	
;	TokenNameSEMICOLON	
private	TokenNameprivate	
boolean	TokenNameboolean	
targetCorrectness	TokenNameIdentifier	 target Correctness
=	TokenNameEQUAL	
false	TokenNamefalse	
;	TokenNameSEMICOLON	
//* If true, only show example which contain the target feature 	TokenNameCOMMENT_LINE	* If true, only show example which contain the target feature 
private	TokenNameprivate	
boolean	TokenNameboolean	
filterOnFeature	TokenNameIdentifier	 filter On Feature
=	TokenNameEQUAL	
false	TokenNamefalse	
;	TokenNameSEMICOLON	
private	TokenNameprivate	
Feature	TokenNameIdentifier	 Feature
targetFeature	TokenNameIdentifier	 target Feature
=	TokenNameEQUAL	
null	TokenNamenull	
;	TokenNameSEMICOLON	
@	TokenNameAT	
Override	TokenNameIdentifier	 Override
public	TokenNamepublic	
void	TokenNamevoid	
applyControls	TokenNameIdentifier	 apply Controls
(	TokenNameLPAREN	
ViewerControls	TokenNameIdentifier	 Viewer Controls
controls	TokenNameIdentifier	 controls
)	TokenNameRPAREN	
{	TokenNameLBRACE	
DataControls	TokenNameIdentifier	 Data Controls
dc	TokenNameIdentifier	 dc
=	TokenNameEQUAL	
(	TokenNameLPAREN	
DataControls	TokenNameIdentifier	 Data Controls
)	TokenNameRPAREN	
controls	TokenNameIdentifier	 controls
;	TokenNameSEMICOLON	
if	TokenNameif	
(	TokenNameLPAREN	
dc	TokenNameIdentifier	 dc
.	TokenNameDOT	
allButton	TokenNameIdentifier	 all Button
.	TokenNameDOT	
isSelected	TokenNameIdentifier	 is Selected
(	TokenNameLPAREN	
)	TokenNameRPAREN	
)	TokenNameRPAREN	
filterOnCorrectness	TokenNameIdentifier	 filter On Correctness
=	TokenNameEQUAL	
false	TokenNamefalse	
;	TokenNameSEMICOLON	
else	TokenNameelse	
{	TokenNameLBRACE	
filterOnCorrectness	TokenNameIdentifier	 filter On Correctness
=	TokenNameEQUAL	
true	TokenNametrue	
;	TokenNameSEMICOLON	
targetCorrectness	TokenNameIdentifier	 target Correctness
=	TokenNameEQUAL	
dc	TokenNameIdentifier	 dc
.	TokenNameDOT	
correctButton	TokenNameIdentifier	 correct Button
.	TokenNameDOT	
isSelected	TokenNameIdentifier	 is Selected
(	TokenNameLPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
}	TokenNameRBRACE	
filterOnFeature	TokenNameIdentifier	 filter On Feature
=	TokenNameEQUAL	
dc	TokenNameIdentifier	 dc
.	TokenNameDOT	
filterOnFeatureBox	TokenNameIdentifier	 filter On Feature Box
.	TokenNameDOT	
isSelected	TokenNameIdentifier	 is Selected
(	TokenNameLPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
targetFeature	TokenNameIdentifier	 target Feature
=	TokenNameEQUAL	
dc	TokenNameIdentifier	 dc
.	TokenNameDOT	
targetFeature	TokenNameIdentifier	 target Feature
;	TokenNameSEMICOLON	
// setContent here is incorrect - we want to bypass 	TokenNameCOMMENT_LINE	setContent here is incorrect - we want to bypass 
// any caching and force an update 	TokenNameCOMMENT_LINE	any caching and force an update 
receiveContent	TokenNameIdentifier	 receive Content
(	TokenNameLPAREN	
cd	TokenNameIdentifier	 cd
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
revalidate	TokenNameIdentifier	 revalidate
(	TokenNameLPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
}	TokenNameRBRACE	
@	TokenNameAT	
Override	TokenNameIdentifier	 Override
public	TokenNamepublic	
JComponent	TokenNameIdentifier	 J Component
componentFor	TokenNameIdentifier	 component For
(	TokenNameLPAREN	
Object	TokenNameIdentifier	 Object
o	TokenNameIdentifier	 o
)	TokenNameRPAREN	
{	TokenNameLBRACE	
cd	TokenNameIdentifier	 cd
=	TokenNameEQUAL	
(	TokenNameLPAREN	
MultiClassifiedDataset	TokenNameIdentifier	 Multi Classified Dataset
)	TokenNameRPAREN	
o	TokenNameIdentifier	 o
;	TokenNameSEMICOLON	
JTable	TokenNameIdentifier	 J Table
jtable	TokenNameIdentifier	 jtable
=	TokenNameEQUAL	
new	TokenNamenew	
JTable	TokenNameIdentifier	 J Table
(	TokenNameLPAREN	
new	TokenNamenew	
MyTableModel	TokenNameIdentifier	 My Table Model
(	TokenNameLPAREN	
filteredMultiClassifiedDataset	TokenNameIdentifier	 filtered Multi Classified Dataset
(	TokenNameLPAREN	
)	TokenNameRPAREN	
)	TokenNameRPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
jtable	TokenNameIdentifier	 jtable
.	TokenNameDOT	
setDefaultRenderer	TokenNameIdentifier	 set Default Renderer
(	TokenNameLPAREN	
MultiExample	TokenNameIdentifier	 Multi Example
.	TokenNameDOT	
class	TokenNameclass	
,	TokenNameCOMMA	
new	TokenNamenew	
TableCellRenderer	TokenNameIdentifier	 Table Cell Renderer
(	TokenNameLPAREN	
)	TokenNameRPAREN	
{	TokenNameLBRACE	
@	TokenNameAT	
Override	TokenNameIdentifier	 Override
public	TokenNamepublic	
Component	TokenNameIdentifier	 Component
getTableCellRendererComponent	TokenNameIdentifier	 get Table Cell Renderer Component
(	TokenNameLPAREN	
JTable	TokenNameIdentifier	 J Table
table	TokenNameIdentifier	 table
,	TokenNameCOMMA	
Object	TokenNameIdentifier	 Object
value	TokenNameIdentifier	 value
,	TokenNameCOMMA	
boolean	TokenNameboolean	
isSelected	TokenNameIdentifier	 is Selected
,	TokenNameCOMMA	
boolean	TokenNameboolean	
hasFocus	TokenNameIdentifier	 has Focus
,	TokenNameCOMMA	
int	TokenNameint	
row	TokenNameIdentifier	 row
,	TokenNameCOMMA	
int	TokenNameint	
column	TokenNameIdentifier	 column
)	TokenNameRPAREN	
{	TokenNameLBRACE	
return	TokenNamereturn	
GUI	TokenNameIdentifier	 GUI
.	TokenNameDOT	
conciseMultiExampleRendererComponent	TokenNameIdentifier	 concise Multi Example Renderer Component
(	TokenNameLPAREN	
(	TokenNameLPAREN	
MultiExample	TokenNameIdentifier	 Multi Example
)	TokenNameRPAREN	
value	TokenNameIdentifier	 value
,	TokenNameCOMMA	
60	TokenNameIntegerLiteral	
,	TokenNameCOMMA	
isSelected	TokenNameIdentifier	 is Selected
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
}	TokenNameRBRACE	
}	TokenNameRBRACE	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
monitorSelections	TokenNameIdentifier	 monitor Selections
(	TokenNameLPAREN	
jtable	TokenNameIdentifier	 jtable
,	TokenNameCOMMA	
1	TokenNameIntegerLiteral	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
JScrollPane	TokenNameIdentifier	 J Scroll Pane
scrollpane	TokenNameIdentifier	 scrollpane
=	TokenNameEQUAL	
new	TokenNamenew	
JScrollPane	TokenNameIdentifier	 J Scroll Pane
(	TokenNameLPAREN	
jtable	TokenNameIdentifier	 jtable
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
scrollpane	TokenNameIdentifier	 scrollpane
.	TokenNameDOT	
setHorizontalScrollBarPolicy	TokenNameIdentifier	 set Horizontal Scroll Bar Policy
(	TokenNameLPAREN	
ScrollPaneConstants	TokenNameIdentifier	 Scroll Pane Constants
.	TokenNameDOT	
HORIZONTAL_SCROLLBAR_AS_NEEDED	TokenNameIdentifier	 HORIZONTAL  SCROLLBAR  AS  NEEDED
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
return	TokenNamereturn	
scrollpane	TokenNameIdentifier	 scrollpane
;	TokenNameSEMICOLON	
}	TokenNameRBRACE	
private	TokenNameprivate	
MultiClassifiedDataset	TokenNameIdentifier	 Multi Classified Dataset
filteredMultiClassifiedDataset	TokenNameIdentifier	 filtered Multi Classified Dataset
(	TokenNameLPAREN	
)	TokenNameRPAREN	
{	TokenNameLBRACE	
if	TokenNameif	
(	TokenNameLPAREN	
!	TokenNameNOT	
filterOnCorrectness	TokenNameIdentifier	 filter On Correctness
&&	TokenNameAND_AND	
!	TokenNameNOT	
filterOnFeature	TokenNameIdentifier	 filter On Feature
)	TokenNameRPAREN	
{	TokenNameLBRACE	
return	TokenNamereturn	
cd	TokenNameIdentifier	 cd
;	TokenNameSEMICOLON	
}	TokenNameRBRACE	
else	TokenNameelse	
{	TokenNameLBRACE	
MultiDataset	TokenNameIdentifier	 Multi Dataset
filteredData	TokenNameIdentifier	 filtered Data
=	TokenNameEQUAL	
new	TokenNamenew	
MultiDataset	TokenNameIdentifier	 Multi Dataset
(	TokenNameLPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
ProgressCounter	TokenNameIdentifier	 Progress Counter
pc	TokenNameIdentifier	 pc
=	TokenNameEQUAL	
new	TokenNamenew	
ProgressCounter	TokenNameIdentifier	 Progress Counter
(	TokenNameLPAREN	
"classifying for ClassifiedDataset"	TokenNameStringLiteral	classifying for ClassifiedDataset
,	TokenNameCOMMA	
"example"	TokenNameStringLiteral	example
,	TokenNameCOMMA	
filteredData	TokenNameIdentifier	 filtered Data
.	TokenNameDOT	
size	TokenNameIdentifier	 size
(	TokenNameLPAREN	
)	TokenNameRPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
for	TokenNamefor	
(	TokenNameLPAREN	
Iterator	TokenNameIdentifier	 Iterator
<	TokenNameLESS	
MultiExample	TokenNameIdentifier	 Multi Example
>	TokenNameGREATER	
i	TokenNameIdentifier	 i
=	TokenNameEQUAL	
cd	TokenNameIdentifier	 cd
.	TokenNameDOT	
dataset	TokenNameIdentifier	 dataset
.	TokenNameDOT	
multiIterator	TokenNameIdentifier	 multi Iterator
(	TokenNameLPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
i	TokenNameIdentifier	 i
.	TokenNameDOT	
hasNext	TokenNameIdentifier	 has Next
(	TokenNameLPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
)	TokenNameRPAREN	
{	TokenNameLBRACE	
MultiExample	TokenNameIdentifier	 Multi Example
e	TokenNameIdentifier	 e
=	TokenNameEQUAL	
i	TokenNameIdentifier	 i
.	TokenNameDOT	
next	TokenNameIdentifier	 next
(	TokenNameLPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
boolean	TokenNameboolean	
pass1	TokenNameIdentifier	 pass1
=	TokenNameEQUAL	
true	TokenNametrue	
;	TokenNameSEMICOLON	
if	TokenNameif	
(	TokenNameLPAREN	
filterOnCorrectness	TokenNameIdentifier	 filter On Correctness
)	TokenNameRPAREN	
pass1	TokenNameIdentifier	 pass1
=	TokenNameEQUAL	
targetCorrectness	TokenNameIdentifier	 target Correctness
==	TokenNameEQUAL_EQUAL	
cd	TokenNameIdentifier	 cd
.	TokenNameDOT	
classifier	TokenNameIdentifier	 classifier
.	TokenNameDOT	
multiLabelClassification	TokenNameIdentifier	 multi Label Classification
(	TokenNameLPAREN	
e	TokenNameIdentifier	 e
)	TokenNameRPAREN	
.	TokenNameDOT	
isCorrect	TokenNameIdentifier	 is Correct
(	TokenNameLPAREN	
e	TokenNameIdentifier	 e
.	TokenNameDOT	
getMultiLabel	TokenNameIdentifier	 get Multi Label
(	TokenNameLPAREN	
)	TokenNameRPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
boolean	TokenNameboolean	
pass2	TokenNameIdentifier	 pass2
=	TokenNameEQUAL	
true	TokenNametrue	
;	TokenNameSEMICOLON	
if	TokenNameif	
(	TokenNameLPAREN	
filterOnFeature	TokenNameIdentifier	 filter On Feature
)	TokenNameRPAREN	
pass2	TokenNameIdentifier	 pass2
=	TokenNameEQUAL	
targetFeature	TokenNameIdentifier	 target Feature
==	TokenNameEQUAL_EQUAL	
null	TokenNamenull	
||	TokenNameOR_OR	
e	TokenNameIdentifier	 e
.	TokenNameDOT	
getWeight	TokenNameIdentifier	 get Weight
(	TokenNameLPAREN	
targetFeature	TokenNameIdentifier	 target Feature
)	TokenNameRPAREN	
>	TokenNameGREATER	
0	TokenNameIntegerLiteral	
;	TokenNameSEMICOLON	
if	TokenNameif	
(	TokenNameLPAREN	
pass1	TokenNameIdentifier	 pass1
&&	TokenNameAND_AND	
pass2	TokenNameIdentifier	 pass2
)	TokenNameRPAREN	
{	TokenNameLBRACE	
filteredData	TokenNameIdentifier	 filtered Data
.	TokenNameDOT	
addMulti	TokenNameIdentifier	 add Multi
(	TokenNameLPAREN	
e	TokenNameIdentifier	 e
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
}	TokenNameRBRACE	
pc	TokenNameIdentifier	 pc
.	TokenNameDOT	
progress	TokenNameIdentifier	 progress
(	TokenNameLPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
}	TokenNameRBRACE	
pc	TokenNameIdentifier	 pc
.	TokenNameDOT	
finished	TokenNameIdentifier	 finished
(	TokenNameLPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
return	TokenNamereturn	
new	TokenNamenew	
MultiClassifiedDataset	TokenNameIdentifier	 Multi Classified Dataset
(	TokenNameLPAREN	
cd	TokenNameIdentifier	 cd
.	TokenNameDOT	
classifier	TokenNameIdentifier	 classifier
,	TokenNameCOMMA	
filteredData	TokenNameIdentifier	 filtered Data
,	TokenNameCOMMA	
cd	TokenNameIdentifier	 cd
.	TokenNameDOT	
index	TokenNameIdentifier	 index
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
}	TokenNameRBRACE	
}	TokenNameRBRACE	
/** models the data in the RandomAccessDataset of the ClassifiedDataset */	TokenNameCOMMENT_JAVADOC	 models the data in the RandomAccessDataset of the ClassifiedDataset 
private	TokenNameprivate	
class	TokenNameclass	
MyTableModel	TokenNameIdentifier	 My Table Model
extends	TokenNameextends	
AbstractTableModel	TokenNameIdentifier	 Abstract Table Model
{	TokenNameLBRACE	
static	TokenNamestatic	
final	TokenNamefinal	
long	TokenNamelong	
serialVersionUID	TokenNameIdentifier	 serial Version UID
=	TokenNameEQUAL	
20080130L	TokenNameLongLiteral	
;	TokenNameSEMICOLON	
private	TokenNameprivate	
MultiClassifiedDataset	TokenNameIdentifier	 Multi Classified Dataset
cd	TokenNameIdentifier	 cd
;	TokenNameSEMICOLON	
public	TokenNamepublic	
MyTableModel	TokenNameIdentifier	 My Table Model
(	TokenNameLPAREN	
MultiClassifiedDataset	TokenNameIdentifier	 Multi Classified Dataset
cd	TokenNameIdentifier	 cd
)	TokenNameRPAREN	
{	TokenNameLBRACE	
this	TokenNamethis	
.	TokenNameDOT	
cd	TokenNameIdentifier	 cd
=	TokenNameEQUAL	
cd	TokenNameIdentifier	 cd
;	TokenNameSEMICOLON	
}	TokenNameRBRACE	
@	TokenNameAT	
Override	TokenNameIdentifier	 Override
public	TokenNamepublic	
int	TokenNameint	
getRowCount	TokenNameIdentifier	 get Row Count
(	TokenNameLPAREN	
)	TokenNameRPAREN	
{	TokenNameLBRACE	
return	TokenNamereturn	
cd	TokenNameIdentifier	 cd
.	TokenNameDOT	
dataset	TokenNameIdentifier	 dataset
.	TokenNameDOT	
size	TokenNameIdentifier	 size
(	TokenNameLPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
}	TokenNameRBRACE	
@	TokenNameAT	
Override	TokenNameIdentifier	 Override
public	TokenNamepublic	
int	TokenNameint	
getColumnCount	TokenNameIdentifier	 get Column Count
(	TokenNameLPAREN	
)	TokenNameRPAREN	
{	TokenNameLBRACE	
return	TokenNamereturn	
2	TokenNameIntegerLiteral	
;	TokenNameSEMICOLON	
}	TokenNameRBRACE	
// predicted, actual, instance 	TokenNameCOMMENT_LINE	predicted, actual, instance 
@	TokenNameAT	
Override	TokenNameIdentifier	 Override
public	TokenNamepublic	
Object	TokenNameIdentifier	 Object
getValueAt	TokenNameIdentifier	 get Value At
(	TokenNameLPAREN	
int	TokenNameint	
row	TokenNameIdentifier	 row
,	TokenNameCOMMA	
int	TokenNameint	
col	TokenNameIdentifier	 col
)	TokenNameRPAREN	
{	TokenNameLBRACE	
if	TokenNameif	
(	TokenNameLPAREN	
col	TokenNameIdentifier	 col
==	TokenNameEQUAL_EQUAL	
0	TokenNameIntegerLiteral	
)	TokenNameRPAREN	
return	TokenNamereturn	
cd	TokenNameIdentifier	 cd
.	TokenNameDOT	
classifier	TokenNameIdentifier	 classifier
.	TokenNameDOT	
multiLabelClassification	TokenNameIdentifier	 multi Label Classification
(	TokenNameLPAREN	
cd	TokenNameIdentifier	 cd
.	TokenNameDOT	
dataset	TokenNameIdentifier	 dataset
.	TokenNameDOT	
getMultiExample	TokenNameIdentifier	 get Multi Example
(	TokenNameLPAREN	
row	TokenNameIdentifier	 row
)	TokenNameRPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
else	TokenNameelse	
if	TokenNameif	
(	TokenNameLPAREN	
col	TokenNameIdentifier	 col
==	TokenNameEQUAL_EQUAL	
1	TokenNameIntegerLiteral	
)	TokenNameRPAREN	
return	TokenNamereturn	
cd	TokenNameIdentifier	 cd
.	TokenNameDOT	
dataset	TokenNameIdentifier	 dataset
.	TokenNameDOT	
getMultiExample	TokenNameIdentifier	 get Multi Example
(	TokenNameLPAREN	
row	TokenNameIdentifier	 row
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
else	TokenNameelse	
throw	TokenNamethrow	
new	TokenNamenew	
IllegalArgumentException	TokenNameIdentifier	 Illegal Argument Exception
(	TokenNameLPAREN	
"illegal col "	TokenNameStringLiteral	illegal col 
+	TokenNamePLUS	
col	TokenNameIdentifier	 col
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
}	TokenNameRBRACE	
@	TokenNameAT	
Override	TokenNameIdentifier	 Override
public	TokenNamepublic	
String	TokenNameIdentifier	 String
getColumnName	TokenNameIdentifier	 get Column Name
(	TokenNameLPAREN	
int	TokenNameint	
col	TokenNameIdentifier	 col
)	TokenNameRPAREN	
{	TokenNameLBRACE	
if	TokenNameif	
(	TokenNameLPAREN	
col	TokenNameIdentifier	 col
==	TokenNameEQUAL_EQUAL	
0	TokenNameIntegerLiteral	
)	TokenNameRPAREN	
return	TokenNamereturn	
"Prediction"	TokenNameStringLiteral	Prediction
;	TokenNameSEMICOLON	
else	TokenNameelse	
if	TokenNameif	
(	TokenNameLPAREN	
col	TokenNameIdentifier	 col
==	TokenNameEQUAL_EQUAL	
1	TokenNameIntegerLiteral	
)	TokenNameRPAREN	
return	TokenNamereturn	
"MultiExample"	TokenNameStringLiteral	MultiExample
;	TokenNameSEMICOLON	
else	TokenNameelse	
throw	TokenNamethrow	
new	TokenNamenew	
IllegalArgumentException	TokenNameIdentifier	 Illegal Argument Exception
(	TokenNameLPAREN	
"illegal col "	TokenNameStringLiteral	illegal col 
+	TokenNamePLUS	
col	TokenNameIdentifier	 col
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
}	TokenNameRBRACE	
}	TokenNameRBRACE	
}	TokenNameRBRACE	
static	TokenNamestatic	
public	TokenNamepublic	
class	TokenNameclass	
ExplanationViewer	TokenNameIdentifier	 Explanation Viewer
extends	TokenNameextends	
ComponentViewer	TokenNameIdentifier	 Component Viewer
{	TokenNameLBRACE	
static	TokenNamestatic	
final	TokenNamefinal	
long	TokenNamelong	
serialVersionUID	TokenNameIdentifier	 serial Version UID
=	TokenNameEQUAL	
20080130L	TokenNameLongLiteral	
;	TokenNameSEMICOLON	
Explanation	TokenNameIdentifier	 Explanation
ex	TokenNameIdentifier	 ex
;	TokenNameSEMICOLON	
public	TokenNamepublic	
ExplanationViewer	TokenNameIdentifier	 Explanation Viewer
(	TokenNameLPAREN	
Explanation	TokenNameIdentifier	 Explanation
ex	TokenNameIdentifier	 ex
)	TokenNameRPAREN	
{	TokenNameLBRACE	
this	TokenNamethis	
.	TokenNameDOT	
ex	TokenNameIdentifier	 ex
=	TokenNameEQUAL	
ex	TokenNameIdentifier	 ex
;	TokenNameSEMICOLON	
setContent	TokenNameIdentifier	 set Content
(	TokenNameLPAREN	
ex	TokenNameIdentifier	 ex
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
}	TokenNameRBRACE	
@	TokenNameAT	
Override	TokenNameIdentifier	 Override
public	TokenNamepublic	
boolean	TokenNameboolean	
canReceive	TokenNameIdentifier	 can Receive
(	TokenNameLPAREN	
Object	TokenNameIdentifier	 Object
o	TokenNameIdentifier	 o
)	TokenNameRPAREN	
{	TokenNameLBRACE	
return	TokenNamereturn	
o	TokenNameIdentifier	 o
instanceof	TokenNameinstanceof	
Explanation	TokenNameIdentifier	 Explanation
;	TokenNameSEMICOLON	
}	TokenNameRBRACE	
@	TokenNameAT	
Override	TokenNameIdentifier	 Override
public	TokenNamepublic	
JComponent	TokenNameIdentifier	 J Component
componentFor	TokenNameIdentifier	 component For
(	TokenNameLPAREN	
Object	TokenNameIdentifier	 Object
o	TokenNameIdentifier	 o
)	TokenNameRPAREN	
{	TokenNameLBRACE	
ex	TokenNameIdentifier	 ex
=	TokenNameEQUAL	
(	TokenNameLPAREN	
Explanation	TokenNameIdentifier	 Explanation
)	TokenNameRPAREN	
o	TokenNameIdentifier	 o
;	TokenNameSEMICOLON	
JScrollPane	TokenNameIdentifier	 J Scroll Pane
p	TokenNameIdentifier	 p
=	TokenNameEQUAL	
new	TokenNamenew	
JScrollPane	TokenNameIdentifier	 J Scroll Pane
(	TokenNameLPAREN	
ex	TokenNameIdentifier	 ex
.	TokenNameDOT	
getExplanation	TokenNameIdentifier	 get Explanation
(	TokenNameLPAREN	
)	TokenNameRPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
return	TokenNamereturn	
p	TokenNameIdentifier	 p
;	TokenNameSEMICOLON	
}	TokenNameRBRACE	
}	TokenNameRBRACE	
/** Viewer for a classified dataset */	TokenNameCOMMENT_JAVADOC	 Viewer for a classified dataset 
static	TokenNamestatic	
private	TokenNameprivate	
class	TokenNameclass	
MyViewer	TokenNameIdentifier	 My Viewer
extends	TokenNameextends	
ComponentViewer	TokenNameIdentifier	 Component Viewer
{	TokenNameLBRACE	
static	TokenNamestatic	
final	TokenNamefinal	
long	TokenNamelong	
serialVersionUID	TokenNameIdentifier	 serial Version UID
=	TokenNameEQUAL	
20080130L	TokenNameLongLiteral	
;	TokenNameSEMICOLON	
private	TokenNameprivate	
Viewer	TokenNameIdentifier	 Viewer
instanceViewer	TokenNameIdentifier	 instance Viewer
,	TokenNameCOMMA	
classifierViewer	TokenNameIdentifier	 classifier Viewer
,	TokenNameCOMMA	
explanationViewer	TokenNameIdentifier	 explanation Viewer
;	TokenNameSEMICOLON	
private	TokenNameprivate	
ControlledViewer	TokenNameIdentifier	 Controlled Viewer
dataViewer	TokenNameIdentifier	 data Viewer
;	TokenNameSEMICOLON	
private	TokenNameprivate	
MultiClassifiedDataset	TokenNameIdentifier	 Multi Classified Dataset
cd	TokenNameIdentifier	 cd
;	TokenNameSEMICOLON	
@	TokenNameAT	
Override	TokenNameIdentifier	 Override
public	TokenNamepublic	
JComponent	TokenNameIdentifier	 J Component
componentFor	TokenNameIdentifier	 component For
(	TokenNameLPAREN	
Object	TokenNameIdentifier	 Object
o	TokenNameIdentifier	 o
)	TokenNameRPAREN	
{	TokenNameLBRACE	
cd	TokenNameIdentifier	 cd
=	TokenNameEQUAL	
(	TokenNameLPAREN	
MultiClassifiedDataset	TokenNameIdentifier	 Multi Classified Dataset
)	TokenNameRPAREN	
o	TokenNameIdentifier	 o
;	TokenNameSEMICOLON	
JSplitPane	TokenNameIdentifier	 J Split Pane
left	TokenNameIdentifier	 left
=	TokenNameEQUAL	
new	TokenNamenew	
JSplitPane	TokenNameIdentifier	 J Split Pane
(	TokenNameLPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
left	TokenNameIdentifier	 left
.	TokenNameDOT	
setOrientation	TokenNameIdentifier	 set Orientation
(	TokenNameLPAREN	
JSplitPane	TokenNameIdentifier	 J Split Pane
.	TokenNameDOT	
VERTICAL_SPLIT	TokenNameIdentifier	 VERTICAL  SPLIT
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
left	TokenNameIdentifier	 left
.	TokenNameDOT	
setResizeWeight	TokenNameIdentifier	 set Resize Weight
(	TokenNameLPAREN	
0.75	TokenNameDoubleLiteral	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
dataViewer	TokenNameIdentifier	 data Viewer
=	TokenNameEQUAL	
new	TokenNamenew	
ControlledViewer	TokenNameIdentifier	 Controlled Viewer
(	TokenNameLPAREN	
new	TokenNamenew	
ControlledDataViewer	TokenNameIdentifier	 Controlled Data Viewer
(	TokenNameLPAREN	
)	TokenNameRPAREN	
,	TokenNameCOMMA	
new	TokenNamenew	
DataControls	TokenNameIdentifier	 Data Controls
(	TokenNameLPAREN	
)	TokenNameRPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
dataViewer	TokenNameIdentifier	 data Viewer
.	TokenNameDOT	
setContent	TokenNameIdentifier	 set Content
(	TokenNameLPAREN	
cd	TokenNameIdentifier	 cd
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
left	TokenNameIdentifier	 left
.	TokenNameDOT	
setTopComponent	TokenNameIdentifier	 set Top Component
(	TokenNameLPAREN	
dataViewer	TokenNameIdentifier	 data Viewer
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
instanceViewer	TokenNameIdentifier	 instance Viewer
=	TokenNameEQUAL	
GUI	TokenNameIdentifier	 GUI
.	TokenNameDOT	
newSourcedMultiExampleViewer	TokenNameIdentifier	 new Sourced Multi Example Viewer
(	TokenNameLPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
left	TokenNameIdentifier	 left
.	TokenNameDOT	
setBottomComponent	TokenNameIdentifier	 set Bottom Component
(	TokenNameLPAREN	
instanceViewer	TokenNameIdentifier	 instance Viewer
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
dataViewer	TokenNameIdentifier	 data Viewer
.	TokenNameDOT	
setSuperView	TokenNameIdentifier	 set Super View
(	TokenNameLPAREN	
this	TokenNamethis	
,	TokenNameCOMMA	
"data"	TokenNameStringLiteral	data
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
instanceViewer	TokenNameIdentifier	 instance Viewer
.	TokenNameDOT	
setSuperView	TokenNameIdentifier	 set Super View
(	TokenNameLPAREN	
this	TokenNamethis	
,	TokenNameCOMMA	
"instance"	TokenNameStringLiteral	instance
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
JSplitPane	TokenNameIdentifier	 J Split Pane
right	TokenNameIdentifier	 right
=	TokenNameEQUAL	
new	TokenNamenew	
JSplitPane	TokenNameIdentifier	 J Split Pane
(	TokenNameLPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
right	TokenNameIdentifier	 right
.	TokenNameDOT	
setOrientation	TokenNameIdentifier	 set Orientation
(	TokenNameLPAREN	
JSplitPane	TokenNameIdentifier	 J Split Pane
.	TokenNameDOT	
VERTICAL_SPLIT	TokenNameIdentifier	 VERTICAL  SPLIT
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
right	TokenNameIdentifier	 right
.	TokenNameDOT	
setResizeWeight	TokenNameIdentifier	 set Resize Weight
(	TokenNameLPAREN	
0.75	TokenNameDoubleLiteral	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
classifierViewer	TokenNameIdentifier	 classifier Viewer
=	TokenNameEQUAL	
(	TokenNameLPAREN	
cd	TokenNameIdentifier	 cd
.	TokenNameDOT	
classifier	TokenNameIdentifier	 classifier
instanceof	TokenNameinstanceof	
Visible	TokenNameIdentifier	 Visible
)	TokenNameRPAREN	
?	TokenNameQUESTION	
(	TokenNameLPAREN	
(	TokenNameLPAREN	
Visible	TokenNameIdentifier	 Visible
)	TokenNameRPAREN	
cd	TokenNameIdentifier	 cd
.	TokenNameDOT	
classifier	TokenNameIdentifier	 classifier
)	TokenNameRPAREN	
.	TokenNameDOT	
toGUI	TokenNameIdentifier	 to GUI
(	TokenNameLPAREN	
)	TokenNameRPAREN	
:	TokenNameCOLON	
new	TokenNamenew	
VanillaViewer	TokenNameIdentifier	 Vanilla Viewer
(	TokenNameLPAREN	
cd	TokenNameIdentifier	 cd
.	TokenNameDOT	
classifier	TokenNameIdentifier	 classifier
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
right	TokenNameIdentifier	 right
.	TokenNameDOT	
setTopComponent	TokenNameIdentifier	 set Top Component
(	TokenNameLPAREN	
classifierViewer	TokenNameIdentifier	 classifier Viewer
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
explanationViewer	TokenNameIdentifier	 explanation Viewer
=	TokenNameEQUAL	
new	TokenNamenew	
ExplanationViewer	TokenNameIdentifier	 Explanation Viewer
(	TokenNameLPAREN	
new	TokenNamenew	
Explanation	TokenNameIdentifier	 Explanation
(	TokenNameLPAREN	
"[explanation]"	TokenNameStringLiteral	[explanation]
)	TokenNameRPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
right	TokenNameIdentifier	 right
.	TokenNameDOT	
setBottomComponent	TokenNameIdentifier	 set Bottom Component
(	TokenNameLPAREN	
explanationViewer	TokenNameIdentifier	 explanation Viewer
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
classifierViewer	TokenNameIdentifier	 classifier Viewer
.	TokenNameDOT	
setSuperView	TokenNameIdentifier	 set Super View
(	TokenNameLPAREN	
this	TokenNamethis	
,	TokenNameCOMMA	
"classifier"	TokenNameStringLiteral	classifier
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
explanationViewer	TokenNameIdentifier	 explanation Viewer
.	TokenNameDOT	
setSuperView	TokenNameIdentifier	 set Super View
(	TokenNameLPAREN	
this	TokenNamethis	
,	TokenNameCOMMA	
"explanation"	TokenNameStringLiteral	explanation
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
JSplitPane	TokenNameIdentifier	 J Split Pane
split	TokenNameIdentifier	 split
=	TokenNameEQUAL	
new	TokenNamenew	
JSplitPane	TokenNameIdentifier	 J Split Pane
(	TokenNameLPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
split	TokenNameIdentifier	 split
.	TokenNameDOT	
setOrientation	TokenNameIdentifier	 set Orientation
(	TokenNameLPAREN	
JSplitPane	TokenNameIdentifier	 J Split Pane
.	TokenNameDOT	
HORIZONTAL_SPLIT	TokenNameIdentifier	 HORIZONTAL  SPLIT
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
split	TokenNameIdentifier	 split
.	TokenNameDOT	
setResizeWeight	TokenNameIdentifier	 set Resize Weight
(	TokenNameLPAREN	
0.50	TokenNameDoubleLiteral	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
split	TokenNameIdentifier	 split
.	TokenNameDOT	
setLeftComponent	TokenNameIdentifier	 set Left Component
(	TokenNameLPAREN	
left	TokenNameIdentifier	 left
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
split	TokenNameIdentifier	 split
.	TokenNameDOT	
setRightComponent	TokenNameIdentifier	 set Right Component
(	TokenNameLPAREN	
right	TokenNameIdentifier	 right
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
MultiEvaluation	TokenNameIdentifier	 Multi Evaluation
e	TokenNameIdentifier	 e
=	TokenNameEQUAL	
new	TokenNamenew	
MultiEvaluation	TokenNameIdentifier	 Multi Evaluation
(	TokenNameLPAREN	
cd	TokenNameIdentifier	 cd
.	TokenNameDOT	
dataset	TokenNameIdentifier	 dataset
.	TokenNameDOT	
getMultiSchema	TokenNameIdentifier	 get Multi Schema
(	TokenNameLPAREN	
)	TokenNameRPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
e	TokenNameIdentifier	 e
.	TokenNameDOT	
extend	TokenNameIdentifier	 extend
(	TokenNameLPAREN	
cd	TokenNameIdentifier	 cd
.	TokenNameDOT	
classifier	TokenNameIdentifier	 classifier
,	TokenNameCOMMA	
cd	TokenNameIdentifier	 cd
.	TokenNameDOT	
dataset	TokenNameIdentifier	 dataset
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
Viewer	TokenNameIdentifier	 Viewer
evalViewer	TokenNameIdentifier	 eval Viewer
=	TokenNameEQUAL	
e	TokenNameIdentifier	 e
.	TokenNameDOT	
toGUI	TokenNameIdentifier	 to GUI
(	TokenNameLPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
JTabbedPane	TokenNameIdentifier	 J Tabbed Pane
main	TokenNameIdentifier	 main
=	TokenNameEQUAL	
new	TokenNamenew	
JTabbedPane	TokenNameIdentifier	 J Tabbed Pane
(	TokenNameLPAREN	
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
main	TokenNameIdentifier	 main
.	TokenNameDOT	
add	TokenNameIdentifier	 add
(	TokenNameLPAREN	
"Details"	TokenNameStringLiteral	Details
,	TokenNameCOMMA	
split	TokenNameIdentifier	 split
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
main	TokenNameIdentifier	 main
.	TokenNameDOT	
add	TokenNameIdentifier	 add
(	TokenNameLPAREN	
"Evaluation"	TokenNameStringLiteral	Evaluation
,	TokenNameCOMMA	
evalViewer	TokenNameIdentifier	 eval Viewer
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
evalViewer	TokenNameIdentifier	 eval Viewer
.	TokenNameDOT	
setSuperView	TokenNameIdentifier	 set Super View
(	TokenNameLPAREN	
this	TokenNamethis	
,	TokenNameCOMMA	
"evaluation"	TokenNameStringLiteral	evaluation
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
return	TokenNamereturn	
main	TokenNameIdentifier	 main
;	TokenNameSEMICOLON	
}	TokenNameRBRACE	
/* protected boolean canHandle(int signal,Object argument){ return (signal==OBJECT_SELECTED)&&(argument instanceof MultiExample)|| (signal==OBJECT_SELECTED)&&(argument instanceof Feature); } protected void handle(int signal,Object argument){ if(argument instanceof MultiExample){ MultiExample example=(MultiExample)argument; instanceViewer.setContent(example); explanationViewer.setContent(cd.classifier.getExplanation(example)); revalidate(); }else if(argument instanceof Feature){ DataControls dc=(DataControls)dataViewer.getControls(); dc.targetFeature=(Feature)argument; dc.filterOnFeatureBox.setText(argument.toString()); sendSignal(TEXT_MESSAGE,featureSummary((Feature)argument,cd.index)); } } */	TokenNameCOMMENT_BLOCK	 protected boolean canHandle(int signal,Object argument){ return (signal==OBJECT_SELECTED)&&(argument instanceof MultiExample)|| (signal==OBJECT_SELECTED)&&(argument instanceof Feature); } protected void handle(int signal,Object argument){ if(argument instanceof MultiExample){ MultiExample example=(MultiExample)argument; instanceViewer.setContent(example); explanationViewer.setContent(cd.classifier.getExplanation(example)); revalidate(); }else if(argument instanceof Feature){ DataControls dc=(DataControls)dataViewer.getControls(); dc.targetFeature=(Feature)argument; dc.filterOnFeatureBox.setText(argument.toString()); sendSignal(TEXT_MESSAGE,featureSummary((Feature)argument,cd.index)); } } 
/* private String featureSummary(Feature f,MultiDatasetIndex index){ StringBuffer buf=new StringBuffer(f.toString()); buf.append(" appears in "); buf.append(index.size(f)); buf.append(" examples:"); Map<String,Integer> map=new TreeMap<String,Integer>(); for(int i=0;i<index.size(f);i++){ String label= index.getMultiExample(f,i).getMultiLabel().bestClassName() .toString(); Integer count=map.get(label); if(count==null) map.put(label,(count=new Integer(0))); map.put(label,new Integer(count.intValue()+1)); } for(Iterator<String> j=map.keySet().iterator();j.hasNext();){ String label=j.next(); Integer count=map.get(label); buf.append(" "+count+":"+label); } return buf.toString(); } */	TokenNameCOMMENT_BLOCK	 private String featureSummary(Feature f,MultiDatasetIndex index){ StringBuffer buf=new StringBuffer(f.toString()); buf.append(" appears in "); buf.append(index.size(f)); buf.append(" examples:"); Map<String,Integer> map=new TreeMap<String,Integer>(); for(int i=0;i<index.size(f);i++){ String label= index.getMultiExample(f,i).getMultiLabel().bestClassName() .toString(); Integer count=map.get(label); if(count==null) map.put(label,(count=new Integer(0))); map.put(label,new Integer(count.intValue()+1)); } for(Iterator<String> j=map.keySet().iterator();j.hasNext();){ String label=j.next(); Integer count=map.get(label); buf.append(" "+count+":"+label); } return buf.toString(); } 
}	TokenNameRBRACE	
public	TokenNamepublic	
static	TokenNamestatic	
void	TokenNamevoid	
main	TokenNameIdentifier	 main
(	TokenNameLPAREN	
String	TokenNameIdentifier	 String
[	TokenNameLBRACKET	
]	TokenNameRBRACKET	
args	TokenNameIdentifier	 args
)	TokenNameRPAREN	
{	TokenNameLBRACE	
System	TokenNameIdentifier	 System
.	TokenNameDOT	
out	TokenNameIdentifier	 out
.	TokenNameDOT	
println	TokenNameIdentifier	 println
(	TokenNameLPAREN	
"ClassifiedDataset"	TokenNameStringLiteral	ClassifiedDataset
)	TokenNameRPAREN	
;	TokenNameSEMICOLON	
}	TokenNameRBRACE	
}	TokenNameRBRACE	
