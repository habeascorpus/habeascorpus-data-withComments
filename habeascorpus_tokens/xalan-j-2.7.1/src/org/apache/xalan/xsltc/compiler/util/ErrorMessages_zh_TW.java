...
/* * $Id: ErrorMessages_zh_TW.java 468649 2006-10-28 07:00:55Z minchau $ */	TokenNameCOMMENT_BLOCK	 $Id: ErrorMessages_zh_TW.java 468649 2006-10-28 07:00:55Z minchau $ 
...
ErrorMessages_zh_TW	TokenNameIdentifier	 Error Messages zh  TW
...
,	TokenNameCOMMA	
"???? java org.apache.xalan.xsltc.cmdline.Compile [-o <??>] [-d <directory>] [-j <Jar ?>] [-p <??>] [-n] [-x] [-u] [-v] [-h] { <stylesheet> | -i } OPTIONS -o <??> ???? <??> ???? translet?????translet ??? ??? <stylesheet> ?????? ??????????????? -d <directory> ?? translet ????? -j <jarfile> ? translet ????? jar ??????? ??? <jarfile> ?? -p <package> ??????? translet ?????????? -n ???????????????????? -x ??????????? -u ??? <stylesheet> ??? URL -i ?????? stdin ????? -v ???????? -h ???????? "	TokenNameStringLiteral	???? java org.apache.xalan.xsltc.cmdline.Compile [-o <??>] [-d <directory>] [-j <Jar ?>] [-p <??>] [-n] [-x] [-u] [-v] [-h] { <stylesheet> | -i } OPTIONS -o <??> ???? <??> ???? translet?????translet ??? ??? <stylesheet> ?????? ??????????????? -d <directory> ?? translet ????? -j <jarfile> ? translet ????? jar ??????? ??? <jarfile> ?? -p <package> ??????? translet ?????????? -n ???????????????????? -x ??????????? -u ??? <stylesheet> ??? URL -i ?????? stdin ????? -v ???????? -h ???????? 
}	TokenNameRBRACE	
,	TokenNameCOMMA	
...
,	TokenNameCOMMA	
"???? java org.apache.xalan.xsltc.cmdline.Transform [-j <Jar ?>] [-x] [-n <??>] {-u <document_url> | <??>} <??> [<param1>=<value1> ...] ?? translet <??> ????? <??> ? XML ???translet <class> ?? ???? CLASSPATH ????????? <jarfile> ?? OPTIONS -j <Jar ?> ?????? translet ? Jar ? -x ??????????? -n <??> ???? <??> ??? ??????? -u <document_url> ?? XML ????? URL "	TokenNameStringLiteral	???? java org.apache.xalan.xsltc.cmdline.Transform [-j <Jar ?>] [-x] [-n <??>] {-u <document_url> | <??>} <??> [<param1>=<value1> ...] ?? translet <??> ????? <??> ? XML ???translet <class> ?? ???? CLASSPATH ????????? <jarfile> ?? OPTIONS -j <Jar ?> ?????? translet ? Jar ? -x ??????????? -n <??> ???? <??> ??? ??????? -u <document_url> ?? XML ????? URL 
}	TokenNameRBRACE	
,	TokenNameCOMMA	
...
,	TokenNameCOMMA	
"??? Java ?? ''{0}'' ????????????????"	TokenNameStringLiteral	??? Java ?? ''{0}'' ????????????????
}	TokenNameRBRACE	
,	TokenNameCOMMA	
