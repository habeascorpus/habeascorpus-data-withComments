...
{	TokenNameLBRACE	
/** * Holds all combinations of <i>indicator</i> flags for fast encoding (saves * time on bit manipulation @ encode time) */	TokenNameCOMMENT_JAVADOC	 Holds all combinations of <i>indicator</i> flags for fast encoding (saves time on bit manipulation @ encode time) 
...
/** * Small values (<=3) are stored in the <code>indicator</code> while larger * values are saved for later encoding in the {@link #encodeQueue}. Since * Vint8 will only encode values larger or equal to 4, the values saves for * encoded are transformed to (value - 4).<br> * When a chunk is ready (got 4 values), the {@link #encodeChunk()} * takes control. */	TokenNameCOMMENT_JAVADOC	 Small values (<=3) are stored in the <code>indicator</code> while larger values are saved for later encoding in the {@link #encodeQueue}. Since Vint8 will only encode values larger or equal to 4, the values saves for encoded are transformed to (value - 4).<br> When a chunk is ready (got 4 values), the {@link #encodeChunk()} takes control. 
@	TokenNameAT	
...
ordinal	TokenNameIdentifier	 ordinal
;	TokenNameSEMICOLON	
// If 4 values were encoded thus far, 'flush' them including the indicator. 	TokenNameCOMMENT_LINE	If 4 values were encoded thus far, 'flush' them including the indicator. 
if	TokenNameif	
(	TokenNameLPAREN	
(	TokenNameLPAREN	
ordinal	TokenNameIdentifier	 ordinal
...
{	TokenNameLBRACE	
return	TokenNamereturn	
"FourFlags ("	TokenNameStringLiteral	FourFlags (
+	TokenNamePLUS	
