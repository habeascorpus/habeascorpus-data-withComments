...
Closeable	TokenNameIdentifier	 Closeable
{	TokenNameLBRACE	
/** * The root category (the category with the empty path) always has the * ordinal 0, to which we give a name ROOT_ORDINAL. * getOrdinal() of an empty path will always return ROOT_ORDINAL, and * getCategory(ROOT_ORDINAL) will return the empty path. */	TokenNameCOMMENT_JAVADOC	 The root category (the category with the empty path) always has the ordinal 0, to which we give a name ROOT_ORDINAL. getOrdinal() of an empty path will always return ROOT_ORDINAL, and getCategory(ROOT_ORDINAL) will return the empty path. 
public	TokenNamepublic	
...
;	TokenNameSEMICOLON	
/** * Ordinals are always non-negative, so a negative ordinal can be used to * signify an error. Methods here return INVALID_ORDINAL (-1) in this case. */	TokenNameCOMMENT_JAVADOC	 Ordinals are always non-negative, so a negative ordinal can be used to signify an error. Methods here return INVALID_ORDINAL (-1) in this case. 
public	TokenNamepublic	
final	TokenNamefinal	
static	TokenNamestatic	
int	TokenNameint	
...
IOException	TokenNameIdentifier	 IO Exception
;	TokenNameSEMICOLON	
/** * getPath() returns the path name of the category with the given * ordinal. The path is returned as a new CategoryPath object - to * reuse an existing object, use {@link #getPath(int, CategoryPath)}. * <P> * A null is returned if a category with the given ordinal does not exist. */	TokenNameCOMMENT_JAVADOC	 getPath() returns the path name of the category with the given ordinal. The path is returned as a new CategoryPath object - to reuse an existing object, use {@link #getPath(int, CategoryPath)}. <P> A null is returned if a category with the given ordinal does not exist. 
public	TokenNamepublic	
CategoryPath	TokenNameIdentifier	 Category Path
...
IOException	TokenNameIdentifier	 IO Exception
;	TokenNameSEMICOLON	
/** * getPath() returns the path name of the category with the given * ordinal. The path is written to the given CategoryPath object (which * is cleared first). * <P> * If a category with the given ordinal does not exist, the given * CategoryPath object is not modified, and the method returns * <code>false</code>. Otherwise, the method returns <code>true</code>. */	TokenNameCOMMENT_JAVADOC	 getPath() returns the path name of the category with the given ordinal. The path is written to the given CategoryPath object (which is cleared first). <P> If a category with the given ordinal does not exist, the given CategoryPath object is not modified, and the method returns <code>false</code>. Otherwise, the method returns <code>true</code>. 
public	TokenNamepublic	
...
IOException	TokenNameIdentifier	 IO Exception
;	TokenNameSEMICOLON	
/** * getParentArray() returns an int array of size getSize() listing the * ordinal of the parent category of each category in the taxonomy. * <P> * The caller can hold on to the array it got indefinitely - it is * guaranteed that no-one else will modify it. The other side of the * same coin is that the caller must treat the array it got as read-only * and <B>not modify it</B>, because other callers might have gotten the * same array too (and getParent() calls might be answered from the * same array). * <P> * If you use getParentArray() instead of getParent(), remember that * the array you got is (naturally) not modified after a refresh(), * so you should always call getParentArray() again after a refresh(). * <P> * This method's function is similar to allocating an array of size * getSize() and filling it with getParent() calls, but implementations * are encouraged to implement it much more efficiently, with O(1) * complexity. This can be done, for example, by the implementation * already keeping the parents in an array, and just returning this * array (without any allocation or copying) when requested. */	TokenNameCOMMENT_JAVADOC	 getParentArray() returns an int array of size getSize() listing the ordinal of the parent category of each category in the taxonomy. <P> The caller can hold on to the array it got indefinitely - it is guaranteed that no-one else will modify it. The other side of the same coin is that the caller must treat the array it got as read-only and <B>not modify it</B>, because other callers might have gotten the same array too (and getParent() calls might be answered from the same array). <P> If you use getParentArray() instead of getParent(), remember that the array you got is (naturally) not modified after a refresh(), so you should always call getParentArray() again after a refresh(). <P> This method's function is similar to allocating an array of size getSize() and filling it with getParent() calls, but implementations are encouraged to implement it much more efficiently, with O(1) complexity. This can be done, for example, by the implementation already keeping the parents in an array, and just returning this array (without any allocation or copying) when requested. 
public	TokenNamepublic	
int	TokenNameint	
[	TokenNameLBRACKET	
]	TokenNameRBRACKET	
...
IOException	TokenNameIdentifier	 IO Exception
;	TokenNameSEMICOLON	
/** * Equivalent representations of the taxonomy's parent info, * used internally for efficient computation of facet results: * "youngest child" and "oldest sibling" */	TokenNameCOMMENT_JAVADOC	 Equivalent representations of the taxonomy's parent info, used internally for efficient computation of facet results: "youngest child" and "oldest sibling" 
public	TokenNamepublic	
static	TokenNamestatic	
interface	TokenNameinterface	
ChildrenArrays	TokenNameIdentifier	 Children Arrays
{	TokenNameLBRACE	
/** * getYoungestChildArray() returns an int array of size getSize() * listing the ordinal of the youngest (highest numbered) child * category of each category in the taxonomy. The value for a leaf * category (a category without children) is * <code>INVALID_ORDINAL</code>. */	TokenNameCOMMENT_JAVADOC	 getYoungestChildArray() returns an int array of size getSize() listing the ordinal of the youngest (highest numbered) child category of each category in the taxonomy. The value for a leaf category (a category without children) is <code>INVALID_ORDINAL</code>. 
public	TokenNamepublic	
int	TokenNameint	
[	TokenNameLBRACKET	
]	TokenNameRBRACKET	
...
;	TokenNameSEMICOLON	
/** * Retrieve user committed data. * @see TaxonomyWriter#commit(Map) */	TokenNameCOMMENT_JAVADOC	 Retrieve user committed data. @see TaxonomyWriter#commit(Map) 
public	TokenNamepublic	
Map	TokenNameIdentifier	 Map
<	TokenNameLESS	
...
;	TokenNameSEMICOLON	
/** * Expert: decreases the refCount of this TaxonomyReader instance. * If the refCount drops to 0, then pending changes (if any) can be * committed to the taxonomy index and this reader can be closed. * @throws IOException */	TokenNameCOMMENT_JAVADOC	 Expert: decreases the refCount of this TaxonomyReader instance. If the refCount drops to 0, then pending changes (if any) can be committed to the taxonomy index and this reader can be closed. @throws IOException 
public	TokenNamepublic	
void	TokenNamevoid	
