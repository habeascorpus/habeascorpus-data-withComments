...
;	TokenNameSEMICOLON	
/** * Expert: obtains the ordinal of the field value from the default Lucene * {@link org.apache.lucene.search.FieldCache FieldCache} using getStringIndex() * and reverses the order. * <p> * The native lucene index order is used to assign an ordinal value for each field value. * <p> * Field values (terms) are lexicographically ordered by unicode value, and numbered starting at 1. * <br> * Example of reverse ordinal (rord): * <br>If there were only three field values: "apple","banana","pear" * <br>then rord("apple")=3, rord("banana")=2, ord("pear")=1 * <p> * WARNING: * rord() depends on the position in an index and can thus change * when other documents are inserted or deleted, * or if a MultiSearcher is used. * * @lucene.experimental * * <p><b>NOTE</b>: with the switch in 2.9 to segment-based * searching, if {@link #getValues} is invoked with a * composite (multi-segment) reader, this can easily cause * double RAM usage for the values in the FieldCache. It's * best to switch your application to pass only atomic * (single segment) readers to this API.</p> */	TokenNameCOMMENT_JAVADOC	 Expert: obtains the ordinal of the field value from the default Lucene {@link org.apache.lucene.search.FieldCache FieldCache} using getStringIndex() and reverses the order. <p> The native lucene index order is used to assign an ordinal value for each field value. <p> Field values (terms) are lexicographically ordered by unicode value, and numbered starting at 1. <br> Example of reverse ordinal (rord): <br>If there were only three field values: "apple","banana","pear" <br>then rord("apple")=3, rord("banana")=2, ord("pear")=1 <p> WARNING: rord() depends on the position in an index and can thus change when other documents are inserted or deleted, or if a MultiSearcher is used. * @lucene.experimental * <p><b>NOTE</b>: with the switch in 2.9 to segment-based searching, if {@link #getValues} is invoked with a composite (multi-segment) reader, this can easily cause double RAM usage for the values in the FieldCache. It's best to switch your application to pass only atomic (single segment) readers to this API.</p> 
...
;	TokenNameSEMICOLON	
/** * Contructor for a certain field. * @param field field whose values reverse order is used. */	TokenNameCOMMENT_JAVADOC	 Contructor for a certain field. @param field field whose values reverse order is used. 
public	TokenNamepublic	
ReverseOrdFieldSource	TokenNameIdentifier	 Reverse Ord Field Source
(	TokenNameLPAREN	
String	TokenNameIdentifier	 String
...
"rord("	TokenNameStringLiteral	rord(
+	TokenNamePLUS	
field	TokenNameIdentifier	 field
+	TokenNamePLUS	
...
sindex	TokenNameIdentifier	 sindex
=	TokenNameEQUAL	
FieldCache	TokenNameIdentifier	 Field Cache
.	TokenNameDOT	
